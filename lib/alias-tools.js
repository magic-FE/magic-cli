...
exports.deleteUserAlias = function(key, currentAlias, cb) {
  ...
  jsonOperator.writeFile(
    userSourcePath, { alias: currentAlias },
    function(err) {
      if (err) return console.log(textHelper.error(`Delete alias error ${err.message}`))
...
exports.addUserAlias = function(key, value, cb) {
  ...
  if ((value.charAt(0) === '/' || /^\w:/.test(value)) && !exists(value)) {
